...
**COMPILE VERIFIED (2026-07-18, commit 99780ab):** `cargo check --no-default-features --features="dart"` is clean and `flowy-backup` tests pass — confirmed both locally and on CI (a `Backup Branch Compile Check` GitHub Actions workflow on the fork, run 29663908024, green). The blind Plan-2 code had real compile errors (empty-PB event, dangling attribute, missing `flowy-backup` dep, `ProtoBuf_Enum` on the event, 6-param handler vs lib-dispatch's 5-param cap, etc.) — all fixed. The backup event is now input-less and registered in `flowy-core` via a single bundled `BackupPluginState`.

Still to do on a real dev env: **Task 5 running-app verification** (the actual correctness gate — compiling ≠ the quiesce is safe under live plugin flushes). Plans 3 (Drive/OAuth), 4 (UI), 5 (restore-on-launch) remain unstarted. Note the Flutter side has no generated dart binding for the `BackupWorkspace` event yet (event uses `Flowy_Event` only, not the dart-event codegen) — Plan 4 (UI) will need to wire that.
